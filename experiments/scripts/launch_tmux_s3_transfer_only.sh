...
NUM_WORKERS="${1:-1}"
WORKLOAD="${2:-nano_gpt_4_layers_64_embd}"
COORDINATOR_HOST="${3:-127.0.0.1}"
COORDINATOR_PORT="${4:-50051}"
OPS_PER_CHUNK="${5:-1}"
SPLIT_MODE="${6:-fixed}"

SESSION="${TMUX_SESSION:-zkexp-transfer}"
CONDA_ENV="${CONDA_ENV:-zk}"

# Bucket name only; do not include s3://.
...
if [[ "$SPLIT_MODE" != "fixed" && "$SPLIT_MODE" != "none" ]]; then
    echo "Error: split_mode must be 'fixed' or 'none'."
    exit 1
fi
...
if [[ "$SPLIT_MODE" == "none" ]]; then
    ...
    GROUP_TAG="g${OPS_PER_CHUNK}"
fi

RUN_ID="$(date -u +%Y-%m-%d_%H-%M-%S)_${WORKLOAD}_${GROUP_TAG}_transfer-only"
...
BASE_CMD="cd ${ROOT_DIR} && \
source \$(conda info --base)/etc/profile.d/conda.sh && \
conda activate ${CONDA_ENV} && \
export PYTHONPATH=${ROOT_DIR}:\${PYTHONPATH:-}"

RUNTIME_CFG="coordinator.host=${COORDINATOR_HOST} \
...
worker.job_poll_interval_sec=1 \
worker.send_proofs_to_coordinator=false \
jobs.scheduler=fifo \
jobs.transfer_only=true \
storage.backend=s3 \
storage.s3_bucket=${S3_BUCKET} \
storage.s3_prefix=${S3_PREFIX} \
storage.transfer_prefix=transfer \
storage.proving_cache_enabled=false \
...
tmux send-keys -t "$SESSION:coordinator" \
    "${BASE_CMD} && python -m zkinfer.runtime.coordinator_grpc \
    ${RUNTIME_CFG} 2>&1 | tee ${LOGS_DIR}/coordinator.tmux.log" \
    C-m
...
for i in $(seq 1 "$NUM_WORKERS"); do
    ...
    tmux send-keys -t "$SESSION:worker${i}" \
        "${BASE_CMD} && python -m zkinfer.runtime.worker \
        ${RUNTIME_CFG} worker.worker_id=worker_${i} \
